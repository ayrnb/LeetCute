...
__global__ void reduce_no_warp_conflict(float *input,float *output,int N){
...
int main(){
    ...
    reduce_no_warp_conflict<<<num_blocks,THREADS_PER_BLOCK>>>(d_input,d_output,N);
    ...
    printf("reduce_no_warp_conflict time: %f\n",time);
    if(check(output,result,num_blocks)){
        printf("reduce_no_warp_conflict test pass\n");
    }
    else {
        for(int i=0;i<num_blocks;i++){
            printf("gpu result: %f\n",output[i]);
